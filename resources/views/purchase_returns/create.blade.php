...
<div class="max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Buat Retur Pembelian</h2>
            <p class="text-sm text-gray-500 mt-1">Kembalikan barang ke supplier dan sesuaikan tagihan atau deposit.</p>
        </div>
    ...
    <form action="{{ route('purchase-returns.store') }}" method="POST" id="return-form">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <label for="purchase_order_id" class="block text-xs font-bold text-gray-700 uppercase mb-1">Pilih PO Asli <span class="text-red-500">*</span></label>
                            <select name="purchase_order_id" id="purchase_order_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
                            ...
                            <p class="text-xs text-gray-500 mt-1 flex items-center gap-1">
                                <i class="bi bi-info-circle"></i> Hanya PO status 'received' atau 'completed' yang muncul.
                            </p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-0">
                        ...
                        <div id="instruction-text" class="flex flex-col items-center justify-center py-12 text-gray-400 bg-gray-50/50">
                            ...
                            <p class="text-sm font-medium">Silakan pilih Nomor PO di atas untuk memuat item.</p>
                        </div>
                    ...
                    <div class="p-6 border-t border-gray-100 bg-yellow-50/30">
                        ...
                        <textarea class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" name="notes" id="notes" rows="2" placeholder="Contoh: Barang rusak, kemasan cacat, salah kirim..." required></textarea>
                    </div>
                </div>
            ...
            {{-- ===================================================
                 KOLOM KANAN: OPSI REFUND & AKSI (Span 4)
                 =================================================== --}}
            <div class="lg:col-span-4 space-y-6">
                
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-6">
                    <h3 class="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2 border-b border-gray-100 pb-3">
                        <i class="bi bi-wallet2 text-indigo-500"></i> Metode Pengembalian
                    </h3>

                    <div class="space-y-3">
                        {{-- Opsi 1 --}}
                        <label class="relative flex items-start p-3 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-all has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 has-[:checked]:shadow-sm">
                            ...
                            <div class="ml-3">
                                <span class="block text-sm font-bold text-gray-900">Potong Tagihan PO</span>
                                <span class="block text-xs text-gray-500 mt-0.5 leading-snug">Mengurangi hutang PO ini secara langsung. Pilih jika PO belum lunas.</span>
                            </div>
                        </label>
                        ...
                        <label class="relative flex items-start p-3 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-all has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 has-[:checked]:shadow-sm">
                            <div class="flex h-5 items-center">
                                <input type="radio" name="return_handling_type" value="store_as_deposit" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
                            </div>
                            <div class="ml-3">
                                <span class="block text-sm font-bold text-gray-900">Simpan ke Deposit</span>
                                <span class="block text-xs text-gray-500 mt-0.5 leading-snug">Menjadi saldo deposit supplier. Pilih jika PO sudah lunas/cash.</span>
                            </div>
                        </label>
                    </div>
                    ...
                    <div class="flex flex-col gap-3">
                        <button type="submit" class="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md transition flex justify-center items-center gap-2 group">
                            <i class="bi bi-check-circle group-hover:scale-110 transition-transform"></i> Simpan Retur
                        </button>
                        <a href="{{ route('purchase-returns.index') }}" class="w-full py-3 bg-white border border-gray-300 text-gray-700 text-sm font-bold rounded-lg hover:bg-gray-50 transition text-center shadow-sm">
                            Batal
                        </a>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
                ...
                if (data.items && data.items.length > 0) {
                    ...
                    data.items.forEach((item, index) => {
                        ...
                        if (maxQty > 0) {
                            ...
                            // --- INI BAGIAN PENTING: HTML YANG DI-GENERATE JS HARUS TAILWIND ---
                            const rowHTML = `
                            ...
                            // Validasi Input Max Qty (Real-time feedback)
                    ...
                    if(returnableCount > 0){
                        ...
                        instructionText.html(`
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm text-gray-500">Semua item pada PO ini sudah diretur sepenuhnya.</p>
                            </div>
                        `).show();
                        itemCountBadge.addClass('hidden');
                    }
                    ...
                    instructionText.html(`
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-500 text-sm">Tidak ada item yang ditemukan pada PO ini.</p>
                        </div>
                    `).show();
                }
            })
            .catch(error => {
                ...
                instructionText.html(`
                    <div class="text-center py-8 text-red-500">
                        ...
                        <p class="text-sm">Gagal memuat data. Silakan coba lagi.</p>
                    </div>
                `).show();
            });
    });
    ...
    form.addEventListener('submit', function(e) {
        ...
        document.querySelectorAll('.return-qty-input').forEach(input => {
            if (parseInt(input.val) > 0) hasInput = true;
        });
        
        // Validasi minimal 1 item diisi (Opsional, jika backend handle)
        // if (!hasInput) { ... }
    });
});
</script>
