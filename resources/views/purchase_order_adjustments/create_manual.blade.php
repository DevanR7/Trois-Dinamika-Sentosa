    ...
    <form action="{{ route('purchase-order-adjustments.store.manual') }}" method="POST" id="manual-adjustment-form">
        ...
        <div class="dashboard-card p-0 overflow-hidden">
            ...
            <div class="px-8 py-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 rounded-b-xl">
                <a href="{{ route('purchase-order-adjustments.create') }}" class="px-5 py-2.5 rounded-lg border border-slate-300 text-slate-600 text-sm font-bold hover:bg-white hover:shadow-sm transition-all">
                    Batal
                </a>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function toggleOverpaymentSection() {
        // Hanya tampilkan jika Nota Kredit dipilih
    ...
    if(form) {
        form.addEventListener('submit', function(event) {
            // Unformat AutoNumeric input sebelum submit
            AutoNumeric.getAutoNumericElement(amountFormatted).save();
