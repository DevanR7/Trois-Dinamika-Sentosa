...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-end gap-4 mb-8">
        ...
        <a href="{{ route('admin.loans.create') }}" class="h-[48px] px-8 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md transition-all flex items-center justify-center gap-2 group hover:-translate-y-0.5">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="pr-6 py-4 text-center">
                                <div class="flex justify-center items-center gap-2">
                                    {{-- Detail --}}
                                    <a href="{{ route('admin.loans.show', $loan) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-indigo-600 hover:bg-indigo-50 hover:border-indigo-200 transition shadow-sm" title="Detail & Bayar">
                                        <i class="material-icons text-[16px]">visibility</i>
                                    </a>
                                    
                                    {{-- Edit & Delete (Hanya jika belum ada pembayaran) --}}
                                    @if (!$loan->payments()->exists())
                                        <a href="{{ route('admin.loans.edit', $loan) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-amber-600 hover:bg-amber-50 hover:border-amber-200 transition shadow-sm" title="Edit">
                                            <i class="material-icons text-[16px]">edit</i>
                                        </a>
                                        
                                        {{-- Global Delete Handler --}}
                                        <form action="{{ route('admin.loans.destroy', $loan) }}" method="POST" class="delete-form inline-block">
                                            @csrf @method('DELETE')
                                            <button type="submit" 
