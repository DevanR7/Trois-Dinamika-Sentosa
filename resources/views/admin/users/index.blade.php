    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="dashboard-card group p-0 overflow-hidden hover:ring-1 hover:ring-indigo-300 transition-all duration-300 flex flex-col h-full">
            ...
            <div class="bg-slate-50/50 px-5 py-3 border-t border-slate-100 flex justify-end items-center gap-2">
                @if($user->trashed())
                    @can('manage-users')
                    <form action="{{ route('admin.users.restore', $user->user_id) }}" method="POST" class="form-confirm" 
                ...
                @else
                    @can('manage-users')
                        ...
                        <form action="{{ route('admin.users.approve', $user->user_id) }}" method="POST" class="form-confirm"
                        ...
                        {{-- Edit Button --}}
                        <a href="{{ route('admin.users.edit', $user->user_id) }}" class="text-xs font-bold text-slate-600 bg-white border border-slate-300 hover:border-indigo-300 hover:text-indigo-600 px-3 py-1.5 rounded-lg transition-colors shadow-sm flex items-center gap-1">
                            <i class="material-icons text-[14px]">edit</i> Edit
                        </a>
                        
                        {{-- Delete Button (Merah/Danger) --}}
                        @if(Auth::id() !== $user->user_id)
                        <form action="{{ route('admin.users.destroy', $user->user_id) }}" method="POST" class="form-confirm"
        ...
        <div class="col-span-full">
            <div class="flex flex-col items-center justify-center py-16 text-center bg-white rounded-xl border border-dashed border-slate-300">
                ...
                @if(request('status') === 'deleted')
                    <a href="{{ route('admin.users.index') }}" class="mt-4 text-indigo-600 font-bold text-sm hover:underline">Kembali ke User Aktif</a>
                @endif
            </div>
        </div>
        @endforelse
