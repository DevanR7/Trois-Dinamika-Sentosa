...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-end gap-4 mb-8">
        ...
        <a href="{{ route('admin.sales-returns.create') }}" class="h-[48px] px-8 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 group hover:-translate-y-0.5">
        ...
        <form action="{{ route('admin.sales-returns.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                ...
                <div class="md:col-span-3 flex gap-2">
                    ...
                    <a href="{{ route('admin.sales-returns.index') }}" class="h-[48px] w-[48px] flex items-center justify-center bg-white border border-slate-300 text-slate-500 hover:text-indigo-600 hover:border-indigo-500 font-medium rounded-lg shadow-sm transition" title="Reset">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($salesReturns as $return)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="pl-6 py-4">
                                <a href="{{ route('admin.sales-returns.show', $return->return_id) }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-800 hover:underline font-mono">
