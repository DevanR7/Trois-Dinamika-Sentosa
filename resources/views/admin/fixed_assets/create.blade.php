...
<div class="flex flex-col gap-6 max-w-5xl mx-auto">
    ...
    <form action="{{ route('admin.fixed-assets.store') }}" method="POST" x-data="fixedAssetForm()">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="asset_name" name="asset_name" value="{{ old('asset_name') }}" 
                           ...
                           placeholder="Contoh: MacBook Pro M2 - Divisi IT" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        {{-- Input Hidden (Nilai Murni ke Server) --}}
                        <input type="hidden" id="purchase_cost" name="purchase_cost" x-model="purchaseCost">
                    </div>
                    <p class="text-[10px] text-slate-400 mt-1">Total harga beli termasuk pajak & biaya kirim.</p>
                ...
                <div>
                    ...
                    <select id="fixed_asset_account_id" name="fixed_asset_account_id" class="tom-select" required>
                        ...
                        @foreach($assetAccounts as $acc)
                            <option value="{{ $acc->account_id }}" {{ old('fixed_asset_account_id') == $acc->account_id ? 'selected' : '' }}>
                                {{ $acc->account_number }} - {{ $acc->account_name }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-[10px] text-slate-400 mt-1">Akun Harta untuk mencatat nilai aset ini.</p>
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 mt-1">Akun Kas/Bank yang digunakan untuk membayar.</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="2" class="form-textarea" placeholder="Serial Number, Warna, Kondisi...">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="depreciation_method" name="depreciation_method" class="form-select" required>
                        ...
                        <option value="double_declining" {{ old('depreciation_method') == 'double_declining' ? 'selected' : '' }}>Saldo Menurun Ganda (Double Declining)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 mt-1">Estimasi nilai jual setelah masa pakai habis (bisa 0).</p>
                ...
                <div>
                    ...
                    <select id="accumulated_depreciation_account_id" name="accumulated_depreciation_account_id" class="tom-select" required>
                        <option value="">Pilih Akun Kontra Aset...</option>
                        @foreach($contraAssetAccounts as $acc)
                            <option value="{{ $acc->account_id }}" {{ old('accumulated_depreciation_account_id') == $acc->account_id ? 'selected' : '' }}>
                                {{ $acc->account_number }} - {{ $acc->account_name }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-[10px] text-slate-400 mt-1">Akun penampung total penyusutan.</p>
                ...
                <div>
                    ...
                    <select id="depreciation_expense_account_id" name="depreciation_expense_account_id" class="tom-select" required>
                        ...
                        @foreach($expenseAccounts as $acc)
                            <option value="{{ $acc->account_id }}" {{ old('depreciation_expense_account_id') == $acc->account_id ? 'selected' : '' }}>
                                {{ $acc->account_number }} - {{ $acc->account_name }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-[10px] text-slate-400 mt-1">Akun biaya yang dicatat setiap bulan.</p>
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-slate-100 dark:border-slate-700">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="material-icons text-[18px] mr-2">save</i> Simpan Aset
                </button>
            </div>
        </div>
    </form>
...
<script>
    function fixedAssetForm() {
        return {
            purchaseCost: '{{ old('purchase_cost', 0) }}',
            salvageValue: '{{ old('salvage_value', 0) }}',
            ...
            init() {
                ...
                if(this.$refs.purchase_cost_visual) {
                    ...
                    anCost.set(this.purchaseCost); // Set nilai awal
                ...
                if(this.$refs.salvage_value_visual) {
                    ...
                    anSalvage.set(this.salvageValue); // Set nilai awal
