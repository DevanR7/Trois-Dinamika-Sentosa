...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-6">
        ...
        <a href="{{ route('admin.invoices.create') }}" class="w-full sm:w-auto h-[48px] px-8 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md transition-all flex items-center justify-center gap-2 group hover:-translate-y-0.5">
        ...
        <form action="{{ route('admin.invoices.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                ...
                <div class="md:col-span-2 flex gap-2">
                    ...
                    <a href="{{ route('admin.invoices.index') }}" class="h-[48px] w-[48px] flex items-center justify-center bg-white border border-slate-300 text-slate-500 hover:text-indigo-600 font-medium rounded-lg shadow-sm transition" title="Reset">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 text-sm">
                    ...
                    <div class="lg:col-span-5 flex flex-col gap-2 justify-center">
                        ...
                        <div class="flex gap-2">
                            {{-- Tombol Detail --}}
                            <a href="{{ route('admin.invoices.show', $invoice->invoice_id) }}" class="flex-1 px-3 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-50 transition text-center flex items-center justify-center gap-1">
                                <i class="material-icons text-[16px]">visibility</i> Detail
                            </a>
                            
                            {{-- Tombol PDF --}}
                            <a href="{{ route('admin.invoices.pdf', $invoice->invoice_id) }}" class="flex-1 px-3 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-50 transition text-center flex items-center justify-center gap-1" target="_blank">
                        ...
                        @if($invoice->status == 'draft')
                            <div class="flex gap-2 mt-1">
                                ...
                                <form action="{{ route('admin.invoices.confirm', $invoice->invoice_id) }}" method="POST" class="form-confirm w-full">
                                    @csrf
                                    <button type="submit" 
                                ...
                                {{-- Edit --}}
                                <a href="{{ route('admin.invoices.edit', $invoice->invoice_id) }}" class="px-3 py-2 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg text-xs font-bold hover:bg-amber-100 transition flex items-center justify-center gap-1">
                                    <i class="material-icons text-[16px]">edit</i>
                                </a>

                                {{-- Hapus Draft --}}
                                <form action="{{ route('admin.invoices.destroy', $invoice->invoice_id) }}" method="POST" class="delete-form">
                                    @csrf @method('DELETE')
                        ...
                        @elseif(!in_array($invoice->status, ['cancelled', 'paid']) && $invoice->remaining_balance > 0)
                            {{-- Tombol Bayar --}}
                            <button onclick="location.href='{{ route('admin.invoices.show', $invoice->invoice_id) }}'" class="w-full px-3 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition flex items-center justify-center gap-1 shadow-sm mt-1">
