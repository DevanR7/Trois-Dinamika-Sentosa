...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div class="flex gap-2 w-full sm:w-auto">
            <a href="{{ route('admin.clients.index') }}" class="px-4 py-2 bg-white border border-slate-300 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center gap-2">
                <i class="material-icons text-[18px]">arrow_back</i> Kembali
            </a>
            
            @if(!$client->trashed())
                <a href="{{ route('admin.clients.edit', $client->client_id) }}" class="px-4 py-2 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg text-sm font-bold hover:bg-amber-100 flex items-center gap-2 transition-all shadow-sm">
            ...
            <div class="dashboard-card p-0 overflow-hidden shadow-sm h-full flex flex-col">
                <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                    ...
                    @if($client->salesInvoices()->count() > 0)
                        <a href="{{ route('admin.invoices.index', ['search' => $client->client_name]) }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-800 hover:underline uppercase tracking-wide">
                            Lihat Semua
                        </a>
                    @endif
                </div>
