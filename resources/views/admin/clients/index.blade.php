    ...
    <div class="flex flex-col sm:flex-row justify-between items-end gap-4 mb-8">
        ...
        <div class="flex gap-3 w-full sm:w-auto">
            @if(request('status') === 'deleted')
                <a href="{{ route('admin.clients.index') }}" class="h-[48px] px-6 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center gap-2">
                    <i class="material-icons text-[18px]">arrow_back</i> Kembali ke Aktif
                </a>
            @else
                <a href="{{ route('admin.clients.index', ['status' => 'deleted']) }}" class="h-[48px] px-6 bg-white border border-slate-300 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center gap-2">
                    <i class="material-icons text-[18px]">archive</i> Lihat Arsip
                </a>
                <a href="{{ route('admin.clients.create') }}" class="h-[48px] px-8 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-sm transition-all flex items-center justify-center gap-2">
                        ...
                        <td class="pr-6 py-4 text-center whitespace-nowrap">
                            <div class="flex justify-center items-center gap-1">
                                @if($client->trashed())
                                    ...
                                    <form action="{{ route('admin.clients.restore', $client->client_id) }}" method="POST" class="form-confirm inline-block">
                                        @csrf @method('PATCH')
                                        <button type="submit" 
                                ...
                                @else
                                    ...
                                    @if(!$client->is_approved)
                                        <form action="{{ route('admin.clients.approve', $client->client_id) }}" method="POST" class="form-confirm inline-block">
                                            @csrf @method('PATCH')
                                            <button type="submit" 
                                    ...
                                    {{-- 2. Lock/Unlock --}}
                                    <form action="{{ route('admin.clients.lock', $client->client_id) }}" method="POST" class="form-confirm inline-block">
                                        @csrf @method('PATCH')
                                        <button type="submit" 
                                    ...
                                    {{-- 5. Archive --}}
                                    <form action="{{ route('admin.clients.destroy', $client->client_id) }}" method="POST" class="form-confirm inline-block">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
