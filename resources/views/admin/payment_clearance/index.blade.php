                ...
                <tbody class="bg-white divide-y divide-slate-100">
                    @forelse ($pendingPayments as $payment)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="py-4">
                                @if ($payment instanceof \App\Models\Payment)
                                    <a href="{{ route('admin.clients.show', $payment->salesInvoice->client_id) }}" class="text-sm font-bold text-slate-800 hover:text-indigo-600 transition">
                                        {{ $payment->salesInvoice->client->client_name }}
                                    </a>
                                    <div class="text-xs text-slate-500 mt-0.5 font-mono">
                                        Inv: <a href="{{ route('admin.invoices.show', $payment->invoice_id) }}" class="hover:underline hover:text-indigo-500">{{ $payment->salesInvoice->invoice_number }}</a>
                                    </div>
                                @elseif ($payment instanceof \App\Models\PurchaseOrderPayment)
                                    <a href="{{ route('admin.suppliers.show', $payment->purchaseOrder->supplier_id) }}" class="text-sm font-bold text-slate-800 hover:text-indigo-600 transition">
                                        {{ $payment->purchaseOrder->supplier->supplier_name }}
                                    </a>
                                    <div class="text-xs text-slate-500 mt-0.5 font-mono">
                                        PO: <a href="{{ route('admin.purchase-orders.show', $payment->po_id) }}" class="hover:underline hover:text-indigo-500">{{ $payment->purchaseOrder->po_number }}</a>
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="pr-6 py-4 text-center">
                                <div class="flex justify-center gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                    @if ($payment instanceof \App\Models\Payment)
                                        ...
                                        <form action="{{ route('admin.payment-clearance.sales.approve', $payment->payment_id) }}" method="POST" class="form-confirm inline-block">
                                            @csrf
                                            <button type="submit" 
                                        ...
                                        <form action="{{ route('admin.payment-clearance.sales.reject', $payment->payment_id) }}" method="POST" class="form-confirm inline-block">
                                            @csrf
                                            <button type="submit" 
                                    ...
                                    @elseif ($payment instanceof \App\Models\PurchaseOrderPayment)
                                        ...
                                        <form action="{{ route('admin.payment-clearance.purchase.approve', $payment->payment_id) }}" method="POST" class="form-confirm inline-block">
                                            @csrf
                                            <button type="submit" 
                                        ...
                                        <form action="{{ route('admin.payment-clearance.purchase.reject', $payment->payment_id) }}" method="POST" class="form-confirm inline-block">
                                            @csrf
                                            <button type="submit" 
