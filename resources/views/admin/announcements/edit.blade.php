    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="page-subtitle">Perbarui isi atau target audiens.</p>
            </div>
        ...
        <form action="{{ route('admin.announcements.update', $announcement->id) }}" method="POST">
            ...
            {{-- x-data diinisialisasi dengan nilai dari DB/Old Input --}}
            <div class="card" x-data="{ type: '{{ old('type', $announcement->type) }}' }">
                ...
                <div class="card-body space-y-6">
                        ...
                        <select name="client_ids[]" class="tom-select" multiple placeholder="Cari nama klien...">
                            @foreach($clients as $client)
                                <option value="{{ $client->client_id }}" 
                                    {{ in_array($client->client_id, old('client_ids', $selectedClientIds)) ? 'selected' : '' }}>
                                    {{ $client->client_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                        <div>
                            <span class="text-sm font-bold text-slate-700 dark:text-slate-200">Status Publikasi</span>
                            <p class="text-xs text-slate-500">Aktifkan agar terlihat oleh klien.</p>
                        </div>
