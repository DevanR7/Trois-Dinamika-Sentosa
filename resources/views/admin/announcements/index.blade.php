    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div class="mt-4 sm:mt-0 flex gap-2 w-full sm:w-auto">
            @if(request('status') === 'deleted')
                <a href="{{ route('admin.announcements.index') }}" class="flex-1 sm:flex-none h-[48px] px-6 bg-white border border-slate-300 rounded-lg text-slate-700 text-sm font-bold hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center gap-2">
                    <i class="material-icons text-[18px]">arrow_back</i> Kembali ke Aktif
                </a>
            @else
                <a href="{{ route('admin.announcements.index', ['status' => 'deleted']) }}" class="flex-1 sm:flex-none h-[48px] px-6 bg-white border border-slate-300 rounded-lg text-slate-700 text-sm font-bold hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center gap-2">
                    <i class="material-icons text-[18px]">archive</i> Lihat Arsip
                </a>
                <a href="{{ route('admin.announcements.create') }}" class="flex-1 sm:flex-none h-[48px] px-8 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-sm transition-all flex items-center justify-center gap-2">
                ...
                <div class="p-5 hover:bg-slate-50/50 transition-colors group">
                    <div class="flex justify-between items-start gap-4">
                        ...
                        <div class="flex flex-col items-end gap-2 flex-shrink-0">
                            @if($announcement->trashed())
                                <form action="{{ route('admin.announcements.restore', $announcement->id) }}" method="POST" class="form-restore w-full">
                                    @csrf @method('PATCH')
                                ...
                                <form action="{{ route('admin.announcements.forceDelete', $announcement->id) }}" method="POST" class="form-force-delete w-full">
                                    @csrf @method('DELETE')
                            ...
                            @else
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.announcements.edit', $announcement->id) }}" class="inline-flex items-center p-2 bg-white border border-slate-200 rounded text-amber-600 hover:bg-amber-50 shadow-sm transition text-xs font-medium" title="Edit">
                                        <i class="material-icons text-sm">edit</i>
                                    </a>
                                    <form action="{{ route('admin.announcements.destroy', $announcement->id) }}" method="POST" class="form-delete inline-block">
                                        @csrf @method('DELETE')
