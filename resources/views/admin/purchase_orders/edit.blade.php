...
<div class="max-w-full mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">
                <a href="{{ route('admin.purchase-orders.index') }}" class="hover:text-indigo-600 transition">Pesanan</a>
    ...
    {{-- FORM START --}}
    <form action="{{ route('admin.purchase-orders.update', $purchaseOrder->po_id) }}" method="POST" id="po-form">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div class="dashboard-card p-0 overflow-hidden">
                <div class="bg-slate-50/50 px-6 py-4 border-b border-slate-100">
                    <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <i class="material-icons text-indigo-500 text-sm">edit</i> Informasi Supplier & Tanggal
                    </h3>
                </div>
            ...
            <div class="dashboard-card p-6 border-t-4 border-indigo-500">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea class="form-textarea w-full bg-yellow-50/30 border-yellow-100 focus:border-yellow-400 focus:ring-yellow-200" name="notes" id="notes" rows="4">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                        </div>
                    ...
                    <div class="bg-slate-50/50 rounded-xl p-6 border border-slate-200 space-y-4 sticky top-6">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <input type="text" class="form-input text-xs w-28 text-right h-8 {{ old('apply_rounding_discount', $purchaseOrder->apply_rounding_discount) ? '' : 'hidden' }} po-autonumeric" id="rounding_discount_amount_display" placeholder="Rp">
                ...
                <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-slate-100">
                    <a href="{{ route('admin.purchase-orders.index') }}" class="px-6 py-3 bg-white border border-slate-300 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 transition shadow-sm">
                         Batal
                    </a>
                    <button type="submit" id="submit-btn" class="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-bold shadow-lg hover:shadow-xl transition transform hover:-translate-y-0.5 flex items-center gap-2">
                        <i class="material-icons text-lg">check_circle</i> Update Pesanan
                    </button>
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function initBoundAutoNumeric(displayElement, hiddenElement) {
            ...
            const an = new AutoNumeric(displayElement, { 
                ...
                currencySymbol: '', 
                unformatOnSubmit: false // JANGAN diubah jadi true, karena kita kirim hidden input
            });
        ...
        window.addProductRow = function(data = null) {
            ...
            select.on('select2:select', function(e) {
                ...
                // Only set default price if currently empty
        ...
        function applyBulkDiscountToRows(rows) {
            ...
            if (discounts.length === 0) {
                alert("Format diskon salah. Contoh: 10+5");
                return;
            }
            rows.forEach(tr => {
        ...
        window.calculateTotals = function() {
            ...
            if (checkboxDiscFee.checked) {
                ...
                const amt = parseFloat(inputDiscFeeAmountHidden.value) || 0; // Use hidden value
                
                if (pct > 0) discFeeVal = subtotalGlobal * (pct / 100);
                else if (amt > 0) discFeeVal = amt;
            }

            const roundingVal = checkboxRounding.checked ? (parseFloat(inputRoundingAmountHidden.value) || 0) : 0; // Use hidden
            ...
            const shippingVal = parseFloat(inputShippingHidden.value) || 0; // Use hidden
