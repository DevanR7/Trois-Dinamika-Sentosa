...
<div class="flex flex-col gap-6 pb-20" x-data="purchaseOrderShow()">
        ...
        <div>
            <div class="flex items-center gap-3 mb-1">
                ...
                @php
                    ...
                    // grand_total di DB sudah merupakan Nilai Revisi (340.000).
                    // Jadi Net Payable = Grand Total - Retur Potong Tagihan.
                    // Jangan kurangi Adjustment lagi karena sudah include di Grand Total.
                    // ------------------------------------------------------------------
                    
                    // Total Kewajiban Bersih (Current Bill)
                    $netPayable = $purchaseOrder->grand_total - $purchaseOrder->total_returned;
                    
                    // Sisa Hutang Real
                    $realRemainingBalance = max(0, round($netPayable - $purchaseOrder->amount_paid, 2));
                    ...
                    if ($realRemainingBalance <= 0.01) {
                        ...
                        $paymentClass = 'bg-emerald-100 text-emerald-700 border border-emerald-200';
                    } elseif ($purchaseOrder->amount_paid > 0) {
                        $paymentStatusLabel = 'Parsial';
                        $paymentClass = 'bg-amber-100 text-amber-700 border border-amber-200';
                    } else {
                        $paymentStatusLabel = 'Belum Lunas';
                        $paymentClass = 'bg-rose-100 text-rose-700 border border-rose-200';
                    }
                @endphp
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="p-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <form action="{{ route('admin.purchase-orders.addSupplierInvoice', $purchaseOrder->po_id) }}" method="POST" class="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    <div class="relative w-full sm:w-auto">
                        ...
                        <input type="text" name="supplier_invoice_number" value="{{ $purchaseOrder->supplier_invoice_number }}" class="form-input py-2 pl-9 pr-3 text-xs w-full sm:w-64 border-slate-300 focus:border-indigo-500 rounded-lg shadow-sm" placeholder="Input No. Faktur Supplier">
                    </div>
                    <button type="submit" class="btn btn-sm btn-primary px-3 h-[34px]" title="Simpan No Faktur"><i class="material-icons text-[18px]">save</i></button>
                </form>
            </div>
            ...
            <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{{ $purchaseOrder->supplier->address ?? 'Alamat tidak tersedia' }}</div>
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="card p-4 border-l-4 border-l-indigo-500 flex justify-between items-center shadow-sm hover:shadow-md transition-shadow">
                <div>
                    {{-- Ganti Label agar tidak bingung --}}
                    <div class="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">Total Tagihan (Terkini)</div>
            ...
            <div class="card p-4 border-l-4 border-l-rose-500 flex justify-between items-center shadow-sm hover:shadow-md transition-shadow">
                <div>
                    ...
                    <div class="text-xl font-mono font-bold text-rose-600 dark:text-rose-400">
                        Rp {{ number_format($realRemainingBalance, 0, ',', '.') }}
                    </div>
                </div>
    ...
    {{-- 3. ITEM LIST TABLE (ORIGINAL ITEMS) --}}
    <div class="card overflow-hidden border border-slate-200 dark:border-slate-700">
        <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
            <h3 class="font-bold text-slate-700 dark:text-white flex items-center gap-2">
                <i class="material-icons text-slate-400 text-sm">list</i> Rincian Barang (Aktif)
            </h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tfoot class="bg-slate-50/50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 text-sm">
                    ...
                    <tr><td colspan="6" class="border-b border-dashed border-slate-200 dark:border-slate-700"></td></tr>

                    <tr>
                        <td colspan="5" class="px-6 py-1 text-right text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">DPP (Dasar Pengenaan Pajak)</td>
                        <td class="px-6 py-1 text-right font-mono text-sm font-bold text-slate-700 dark:text-slate-200">{{ number_format($purchaseOrder->dpp, 0, ',', '.') }}</td>
                    </tr>

                    @if($purchaseOrder->use_custom_dpp_factor)
                    <tr>
                        <td colspan="5" class="px-6 py-0 text-right text-[10px] text-indigo-500 italic">*Menggunakan Faktor DPP: {{ $purchaseOrder->custom_dpp_factor == 0.91666667 ? '11/12' : round($purchaseOrder->custom_dpp_factor, 4) }}</td>
                        <td class="px-6 py-0"></td>
                    </tr>
                    @endif

                    <tr>
                        <td colspan="5" class="px-6 py-1 text-right text-xs text-slate-500">PPN @if($purchaseOrder->tax_id) ({{ $purchaseOrder->tax->rate }}%) @else (0%) @endif</td>
                        <td class="px-6 py-1 text-right font-mono text-xs text-slate-600 dark:text-slate-400">+ {{ number_format($purchaseOrder->ppn, 0, ',', '.') }}</td>
                    </tr>

                    <tr>
                        <td colspan="5" class="px-6 py-1 text-right text-xs text-slate-500">Biaya Kirim / Lainnya</td>
                        <td class="px-6 py-1 text-right font-mono text-xs text-slate-600 dark:text-slate-400">+ {{ number_format($purchaseOrder->shipping_amount, 0, ',', '.') }}</td>
                    </tr>

                    {{-- TOTAL REVISI --}}
                    <tr class="bg-slate-100 dark:bg-slate-700/50 font-bold border-y border-slate-200 dark:border-slate-700">
                        <td colspan="5" class="px-6 py-2 text-right text-xs text-slate-700 dark:text-slate-300 uppercase">Total Order (Revisi)</td>
                        <td class="px-6 py-2 text-right font-mono text-slate-700 dark:text-white">Rp {{ number_format($purchaseOrder->grand_total, 0, ',', '.') }}</td>
                    </tr>

                    @if($purchaseOrder->total_returned > 0)
                    <tr>
                        <td colspan="5" class="px-6 py-1 text-right text-xs text-rose-500">Retur (Potong Tagihan)</td>
                        <td class="px-6 py-1 text-right font-mono text-xs text-rose-500">- {{ number_format($purchaseOrder->total_returned, 0, ',', '.') }}</td>
                    </tr>
                    @endif

                    <tr class="bg-indigo-50 dark:bg-indigo-900/20 border-t-2 border-indigo-100 dark:border-indigo-800">
                        <td colspan="5" class="px-6 py-3 text-right text-sm font-bold text-indigo-700 dark:text-indigo-300 uppercase">Total Kewajiban (Net)</td>
                    ...
                    @if($depositReturns > 0)
                    <tr>
                        <td colspan="5" class="px-6 py-1 text-right text-xs text-amber-600 font-bold uppercase">Info: Retur Masuk Deposit</td>
                        <td class="px-6 py-1 text-right font-mono text-xs text-amber-600 font-bold">({{ number_format($depositReturns, 0, ',', '.') }})</td>
                    </tr>
                    <tr><td colspan="6" class="px-6 py-0 text-right text-[10px] text-slate-400 italic">*Nilai ini tersimpan di deposit supplier, gunakan centang "Gunakan Saldo" saat pembayaran.</td></tr>
                    @endif
                </tfoot>
            </table>
        </div>
    ...
    {{-- SECTION BARU: RIWAYAT KOREKSI & AUDIT TRAIL --}}
    @if($purchaseOrder->adjustments->count() > 0)
        <div class="mt-8">
            <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                <i class="material-icons text-amber-500">history_edu</i> Riwayat Koreksi & Revisi
            </h3>

            <div class="space-y-4">
                @foreach($purchaseOrder->adjustments as $adj)
                    <div class="card p-5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm relative overflow-hidden transition-all hover:shadow-md">
                        
                        {{-- Header Card Adjustment --}}
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 pr-24">
                            <div>
                                <div class="text-xs text-slate-400 font-mono mb-1 flex items-center gap-1">
                                    <i class="material-icons text-[14px]">event</i> {{ $adj->created_at->format('d M Y H:i') }} 
                                    <span class="mx-1">•</span> 
                                    <i class="material-icons text-[14px]">person</i> {{ $adj->user->full_name ?? 'System' }}
                                </div>
                                <h4 class="text-sm font-bold text-slate-700 dark:text-white">
                                    Alasan: <span class="font-normal text-slate-600 dark:text-slate-300 italic">"{{ $adj->reason }}"</span>
                                </h4>
                            </div>
                            
                            {{-- Badge Tipe & Nilai (Dipindah ke sini agar tidak nabrak) --}}
                            <div class="flex flex-col items-end gap-1">
                                <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase border
                                    {{ $adj->type == 'credit_note' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-amber-100 text-amber-700 border-amber-200' }}">
                                    {{ $adj->type == 'credit_note' ? 'Nota Kredit (Potongan)' : 'Nota Debit (Tambahan)' }}
                                </span>
                                <div class="text-lg font-mono font-bold {{ $adj->type == 'credit_note' ? 'text-emerald-600' : 'text-amber-600' }}">
                                    {{ $adj->type == 'credit_note' ? '-' : '+' }} Rp {{ number_format($adj->amount, 0, ',', '.') }}
                                </div>
                            </div>
                        </div>

                        {{-- Tabel Snapshot Item Koreksi --}}
                        @if(isset($adj->details['items']) && is_array($adj->details['items']))
                            <div class="overflow-x-auto rounded-lg border border-slate-100 dark:border-slate-700 mt-2">
                                <table class="w-full text-xs text-left">
                                    <thead class="bg-slate-50 dark:bg-slate-700/50 text-slate-500 uppercase font-bold">
                                        <tr>
                                            <th class="px-4 py-2">Produk & Perubahan</th>
                                            <th class="px-4 py-2 text-right">Harga</th>
                                            <th class="px-4 py-2 text-center">Qty</th>
                                            <th class="px-4 py-2 text-right">Subtotal</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700 bg-slate-50/20">
                                        @foreach($adj->details['items'] as $item)
                                            @php
                                                // Logika Deteksi Status Item
                                                $isDeleted = ((float)$item['quantity'] <= 0);
                                                $hasLog = !empty($item['change_logs']);
                                                $oldPrice = $item['old_price'] ?? $item['price'];
                                                $oldQty   = $item['old_quantity'] ?? $item['quantity'];
                                                
                                                // Warna Baris
                                                $rowClass = '';
                                                if ($isDeleted) {
                                                    $rowClass = 'bg-rose-50 dark:bg-rose-900/20'; // Merah jika dihapus
                                                } elseif ($hasLog) {
                                                    $rowClass = 'bg-amber-50/50 dark:bg-amber-900/10'; // Kuning jika ada perubahan
                                                }
                                            @endphp

                                            <tr class="{{ $rowClass }}">
                                                <td class="px-4 py-2 align-top">
                                                    <div class="font-bold {{ $isDeleted ? 'text-rose-500 line-through' : 'text-slate-700 dark:text-slate-200' }}">
                                                        {{ $item['product_name'] }}
                                                    </div>
                                                    ...
                                                    @if($isDeleted)
                                                        <div class="mt-1">
                                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-rose-100 text-rose-600 border border-rose-200">
                                                                <i class="material-icons text-[10px] mr-1">delete</i> DIBATALKAN / DIHAPUS
                                                            </span>
                                                        </div>
                                                    @elseif($hasLog && is_array($item['change_logs']))
                                                        <div class="mt-1 space-y-1">
                                                            @foreach($item['change_logs'] as $log)
                                                                <div class="text-[10px] text-amber-700 bg-amber-100/50 px-1.5 py-0.5 rounded w-fit flex items-center gap-1 border border-amber-100">
                                                                    <i class="material-icons text-[10px]">edit</i> {{ $log }}
                                                                </div>
                                                            @endforeach
                                                        </div>
                                                    @endif
                                                ...
                                                <td class="px-4 py-2 text-right align-top">
                                                    @if(!$isDeleted && $oldPrice != $item['price'])
                                                        <div class="text-[10px] text-slate-400 line-through">{{ number_format($oldPrice, 0, ',', '.') }}</div>
                                                        <div class="text-indigo-600 font-bold">{{ number_format($item['price'], 0, ',', '.') }}</div>
                                                    @else
                                                        <div class="{{ $isDeleted ? 'line-through text-rose-400' : 'text-slate-600 dark:text-slate-300' }}">{{ number_format($item['price'], 0, ',', '.') }}</div>
                                                    @endif
                                                </td>
                                                <td class="px-4 py-2 text-center align-top">
                                                    @if(!$isDeleted && $oldQty != $item['quantity'])
                                                        <span class="text-slate-400 line-through mr-1 text-[10px]">{{ (float)$oldQty }}</span>
                                                        <span class="text-indigo-600 font-bold">{{ (float)$item['quantity'] }}</span>
                                                    @else
                                                        <span class="{{ $isDeleted ? 'line-through text-rose-400' : 'text-slate-700 font-medium' }}">{{ (float)$item['quantity'] }}</span>
                                                    @endif
                                                    <span class="text-[9px] text-slate-400 block">{{ $item['unit_name'] ?? '' }}</span>
                                                </td>
                                                <td class="px-4 py-2 text-right align-top font-medium {{ $isDeleted ? 'line-through text-rose-400' : 'text-slate-800 dark:text-white' }}">
                                                    {{ number_format($item['subtotal'], 0, ',', '.') }}
                                                </td>
                                            </tr>
                                        @endforeach
                        ...
                        {{-- Tombol Hapus Koreksi --}}
                        @can('delete-purchase-orders')
                            <div class="mt-4 flex justify-end border-t border-slate-100 dark:border-slate-700 pt-3">
                                <form action="{{ route('admin.purchase-order-adjustments.destroy', $adj->adjustment_id) }}" method="POST" 
                                      id="form-reverse-adj-{{ $adj->adjustment_id }}">
                                    @csrf @method('DELETE')
                                    <button type="button" @click="confirmReverseAdjustment('form-reverse-adj-{{ $adj->adjustment_id }}')" 
                                            class="text-xs text-rose-500 hover:text-rose-700 hover:underline flex items-center transition-colors">
                                        <i class="material-icons text-[16px] mr-1">delete_forever</i> Batalkan / Hapus Koreksi Ini
                                    </button>
                                </form>
                            </div>
                        @endcan
    ...
    <div x-data="{ activeTab: 'payment' }" class="min-h-[400px]">
        ...
        <div x-show="activeTab === 'payment'" class="animate-enter" x-data="paymentFormLogic()">
            ...
            <div class="flex items-center gap-2 mb-4 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg w-fit">
                ...
                @if($realRemainingBalance > 0.01 && $purchaseOrder->status != 'cancelled')
            ...
            <div x-show="subTab === 'form'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="card p-5 border border-indigo-100 dark:border-indigo-900 shadow-sm relative overflow-hidden">
                        ...
                        <div>
                            ...
                            <div x-show="overpayment > 0" class="mt-2 p-2 bg-emerald-50 text-emerald-700 text-xs rounded border border-emerald-100 flex items-start gap-2" style="display: none;">
                                ...
                                <span>Pembayaran melebihi tagihan. Kelebihan dana sebesar <strong x-text="formatRupiah(overpayment)"></strong> akan otomatis disimpan sebagai <strong>Deposit Supplier</strong>.</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-5 h-fit">
                    <form action="{{ route('admin.purchase-orders.payments.store', $purchaseOrder->po_id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="amount" :value="manualAmount"> 
                                        ...
                                        <div class="flex items-center justify-center w-full">
                                            <label for="proof_of_payment" class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 border-dashed rounded-2xl cursor-pointer bg-slate-50 hover:bg-indigo-50/50 hover:border-indigo-400 dark:bg-slate-800/50 dark:border-slate-600 dark:hover:border-indigo-500 transition-all duration-300 group">
                                                <div class="flex flex-col items-center justify-center pt-5 pb-6 px-4" x-show="!fileName">
                                                    ...
                                                    <p class="text-xs text-slate-500 dark:text-slate-400 text-center"><span class="font-semibold">Klik upload</span> atau drag & drop</p>
                                                </div>
                                                <div class="hidden flex-col items-center justify-center pt-5 pb-6" x-show="fileName" :class="{ 'flex': fileName, 'hidden': !fileName }">
                                                    <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center mb-2"><i class="material-icons text-lg">check</i></div>
                                                ...
                                                <input id="proof_of_payment" name="proof_of_payment" type="file" class="hidden" accept="image/*" @change="fileName = $event.target.files[0] ? $event.target.files[0].name : null" :required="isProofRequired" />
                                            </label>
                                        </div>
                                    </div>
                                </div>
        ...
        <div x-show="activeTab === 'returns'" style="display: none;" class="space-y-8 animate-enter">
            <div class="card overflow-hidden">
                <div class="px-5 py-3 bg-rose-50 dark:bg-rose-900/20 border-b border-rose-100 dark:border-rose-800 flex justify-between items-center">
                    ...
                    @if($purchaseOrder->status === 'completed')
                        <a href="{{ route('admin.purchase-returns.create', ['purchase_order_id' => $purchaseOrder->po_id]) }}" class="btn btn-sm btn-danger border-none shadow-sm"><i class="material-icons text-[16px] mr-1">add</i> Buat Retur</a>
                    @endif
                </div>
                <table class="w-full text-sm text-left">
                    <thead class="text-xs text-slate-500 uppercase bg-white dark:bg-slate-800">
                        <tr><th class="px-5 py-3">No. Retur</th><th class="px-5 py-3">Tanggal</th><th class="px-5 py-3">Tipe Penanganan</th><th class="px-5 py-3 text-right">Nilai Retur</th><th class="px-5 py-3 text-center">Opsi</th></tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($purchaseOrder->returns as $return)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                <td class="px-5 py-3 font-medium text-indigo-600"><a href="{{ route('admin.purchase-returns.show', $return->return_id) }}" class="hover:underline">{{ $return->return_number }}</a></td>
                                <td class="px-5 py-3">{{ $return->return_date->format('d/m/Y') }}</td>
                                <td class="px-5 py-3"><span class="badge {{ $return->return_handling_type == 'deduct_invoice' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700' }}">{{ $return->return_handling_type == 'deduct_invoice' ? 'Potong Tagihan' : 'Simpan Deposit' }}</span></td>
                                <td class="px-5 py-3 text-right font-mono font-bold text-rose-600">{{ number_format($return->total_amount, 0, ',', '.') }}</td>
                                <td class="px-5 py-3 text-center"><a href="{{ route('admin.purchase-returns.show', $return->return_id) }}" class="text-slate-400 hover:text-indigo-600"><i class="material-icons text-[18px]">visibility</i></a></td>
                            </tr>
                        @empty
            ...
            <div class="card overflow-hidden">
                <div class="px-5 py-3 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-100 dark:border-amber-800 flex justify-between items-center">
                    ...
                    @if($purchaseOrder->status === 'completed')
                         <a href="{{ route('admin.purchase-order-adjustments.create', ['purchase_order_id' => $purchaseOrder->po_id]) }}" class="btn btn-sm bg-amber-100 text-amber-700 hover:bg-amber-200 border-none"><i class="material-icons text-[16px] mr-1">tune</i> Adjustment Baru</a>
                    @endif
                </div>
                <table class="w-full text-sm text-left">
                    <thead class="text-xs text-slate-500 uppercase bg-white dark:bg-slate-800">
                        <tr><th class="px-5 py-3">Tanggal</th><th class="px-5 py-3">Jenis</th><th class="px-5 py-3">Alasan</th><th class="px-5 py-3 text-right">Nominal</th><th class="px-5 py-3 text-center">Opsi</th></tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($purchaseOrder->adjustments as $adj)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                <td class="px-5 py-3">{{ $adj->adjustment_date->format('d/m/Y') }}</td>
                                <td class="px-5 py-3"><span class="badge {{ $adj->type == 'credit_note' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">{{ $adj->type == 'credit_note' ? 'Credit Note (Potong Hutang)' : 'Debit Note (Tambah Hutang)' }}</span></td>
                                <td class="px-5 py-3 text-xs text-slate-500 max-w-xs truncate">{{ $adj->reason }}</td>
                                <td class="px-5 py-3 text-right font-mono font-bold">{{ number_format($adj->amount, 0, ',', '.') }}</td>
                                <td class="px-5 py-3 text-center">
                                    <form action="{{ route('admin.purchase-order-adjustments.destroy', $adj->adjustment_id) }}" method="POST" id="form-reverse-adj-{{ $adj->adjustment_id }}">
                                        @csrf @method('DELETE')
                                        <button type="button" @click="confirmReverseAdjustment('form-reverse-adj-{{ $adj->adjustment_id }}')" class="text-slate-400 hover:text-rose-500 transition-colors tooltip" title="Batalkan / Reversal"><i class="material-icons text-[18px]">undo</i></button>
                                    </form>
                                </td>
                            </tr>
                        @empty
        ...
        <div x-show="activeTab === 'notes'" style="display: none;" class="animate-enter">
            <div class="card p-6 bg-yellow-50/50 dark:bg-yellow-900/10 border border-yellow-100 dark:border-yellow-900/30">
                <h4 class="font-bold text-slate-700 dark:text-yellow-400 mb-3 flex items-center gap-2"><i class="material-icons text-amber-500">sticky_note_2</i> Catatan Pesanan</h4>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('purchaseOrderShow', () => ({
            confirmDelete(formId) {
                window.confirmDialog({
                    title: 'Hapus Pembayaran?', text: 'Data pembayaran akan dihapus dan jurnal akuntansi akan di-rollback.', icon: 'warning', confirmText: 'Ya, Hapus', confirmColor: 'danger'
                }).then((result) => { if(result.isConfirmed) document.getElementById(formId).submit(); });
            },
            confirmProcess() {
                window.confirmDialog({ title: 'Proses Pesanan?', text: 'Status akan berubah menjadi "Ordered".', icon: 'question', confirmText: 'Ya, Proses', confirmColor: 'primary'
                }).then((result) => { if(result.isConfirmed) document.getElementById('form-process-order').submit(); });
            },
            confirmReceive() {
                window.confirmDialog({ title: 'Terima Barang?', text: 'Stok akan bertambah dan status menjadi "Selesai".', icon: 'warning', confirmText: 'Ya, Terima Barang', confirmColor: 'success'
                }).then((result) => { if(result.isConfirmed) document.getElementById('form-receive-goods').submit(); });
            },
            confirmReverseAdjustment(formId) {
                window.confirmDialog({ title: 'Batalkan Penyesuaian?', text: 'Jurnal akuntansi terkait akan dihapus (rollback).', icon: 'warning', confirmText: 'Ya, Batalkan', cancelText: 'Tidak', confirmColor: 'danger'
                }).then((result) => { if(result.isConfirmed) document.getElementById(formId).submit(); });
            }
        }));
        ...
        Alpine.data('paymentFormLogic', () => ({
            ...
            // [DATA INJECT DARI CONTROLLER -> PHP -> JS]
            // Pastikan nilai ini sudah dihitung di block @php di atas
            remainingBalance: {{ $realRemainingBalance }},
            supplierBalance: {{ $purchaseOrder->supplier->balance }},
            
            paymentMethods: [
                @foreach($paymentMethods as $pm)
                { id: '{{ $pm->payment_method_id }}', name: '{{ $pm->name }}', config: '{{ $pm->internal_input_config }}' },
                @endforeach
            ],
            useLedger: false, manualAmount: 0, selectedMethodId: '',
            depositUsed: 0, manualNeeded: 0, overpayment: 0,
            needsProof: false, needsReference: false, isProofRequired: false, isReferenceRequired: false,
            ...
            init() {
                ...
                if (methodSelect) {
                    ...
                    new TomSelect(methodSelect, { ...window.defaultTomSelectConfig, onChange: (value) => { this.selectedMethodId = value; this.updateFormConfig(); } });
                }
                const bankSelect = document.getElementById('bank_account_select');
                if (bankSelect) { if (bankSelect.tomselect) bankSelect.tomselect.destroy(); new TomSelect(bankSelect, { ...window.defaultTomSelectConfig }); }
                
                this.$watch('useLedger', () => { this.recalculate(); });
                this.recalculate();
                
                // [LOGIC PINTAR] Jika ada tagihan, otomatis buka tab form
            ...
            setFullAmount() {
                ...
                if (this.useLedger && this.supplierBalance > 0) { depositAllocated = Math.min(this.supplierBalance, this.remainingBalance); }
                const exactManualNeeded = this.remainingBalance - depositAllocated;
                const inputEl = document.getElementById('manual_payment_input');
                if (AutoNumeric.getAutoNumericElement(inputEl)) { AutoNumeric.getAutoNumericElement(inputEl).set(exactManualNeeded); }
                this.manualAmount = exactManualNeeded;
                this.recalculate();
            },

            recalculate() {
                if (this.useLedger && this.supplierBalance > 0) { this.depositUsed = Math.min(this.supplierBalance, this.remainingBalance); } else { this.depositUsed = 0; }
            ...
            updateFormConfig() {
                if (!this.selectedMethodId) { this.needsProof = false; this.needsReference = false; return; }
            ...
            get isValid() {
                ...
                if (this.manualAmount > 0) { if (!this.selectedMethodId) return false; }
                return true;
            },

            formatRupiah(value) { return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(value); }
        }));
    });
</script>
@endpush
