...
@section('content')
<div x-data="purchaseOrderCreate()" class="flex flex-col gap-6">
    
    {{-- HEADER SECTION --}}
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Buat Purchase Order</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Isi formulir di bawah. Data tersimpan otomatis sebagai draft lokal.</p>
        </div>
        
        <div class="flex items-center gap-3">
            {{-- Status Badge --}}
            <div class="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center h-[42px]">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Status: DRAFT</span>
            </div>
    ...
    <form action="{{ route('admin.purchase-orders.store') }}" method="POST" id="poForm" @submit.prevent="submitForm">
        @csrf
        ...
        <div class="card shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden relative">
            ...
            <div x-show="isLoading" class="absolute inset-0 bg-white/80 dark:bg-slate-800/80 z-50 flex items-center justify-center backdrop-blur-sm" style="display: none;">
                <div class="flex flex-col items-center">
                    ...
                    <span class="text-sm font-bold text-indigo-600">Memuat Data...</span>
                </div>
            </div>
                    ...
                    <div class="md:col-span-4">
                        ...
                        <select name="requester_user_id" class="tom-select" x-model="formData.requester_id">
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->user_id }}" {{ Auth::id() == $user->user_id ? 'selected' : '' }}>
                                    {{ $user->full_name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="p-0">
                ...
                {{-- Warning --}}
                <div x-show="!supplierId" class="p-6 text-center text-amber-600 bg-amber-50/50 border-y border-amber-100 italic text-sm">
                    <i class="material-icons align-bottom text-lg mr-1">warning</i>
                    Silakan pilih <strong>Supplier</strong> di bagian atas untuk mulai menambahkan barang.
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div class="p-6 border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                    ...
                    <textarea name="notes" x-model="formData.notes" rows="6" class="form-textarea w-full text-sm leading-relaxed border-slate-200" 
                              placeholder="Tuliskan instruksi pengiriman, kontak penerima, atau catatan penting lainnya di sini..."></textarea>
                </div>
        ...
        <div class="flex justify-between items-center pt-6 pb-10">
            <div>
                <button type="button" @click="clearDraft()" class="text-xs text-rose-500 hover:underline flex items-center gap-1">
                    <i class="material-icons text-sm">delete_sweep</i> Hapus Draft & Reset
                </button>
            </div>
            <div class="flex gap-3">
                <button type="button" @click="saveDraft(true)" class="btn btn-secondary btn-lg border-indigo-200 text-indigo-600 hover:bg-indigo-50">
                    <i class="material-icons mr-2 text-[18px]">save_as</i> Simpan Draft Sementara
                </button>
                <button type="submit" class="btn btn-primary btn-lg px-8 shadow-xl shadow-indigo-500/20 transform hover:-translate-y-1 transition-all">
                    <i class="material-icons mr-2 text-[20px]">check_circle</i> Simpan Final (Draft PO)
                </button>
            </div>
        </div>
    </form>
...
@push('scripts')
<script>
    window.purchaseOrderCreate = function() {
        return {
            items: [],
            supplierId: '',
            filteredProducts: [],
            ...
            init() {
                this.checkDraft();
                this.$watch('items', () => this.saveDraft(false));
                this.$watch('summary', () => this.saveDraft(false));
                this.$watch('formData', () => this.saveDraft(false));
                this.$watch('supplierId', (val) => { this.saveDraft(false); });
            ...
            initSupplierSelect() {
                ...
                if (this.supplierId) {
                    ts.setValue(this.supplierId, true);
                }
            },
            ...
            initTaxSelect() {
                ...
                if (this.summary.tax_id) {
                    ts.setValue(this.summary.tax_id, true);
                }
            },
            ...
            async handleSupplierChange(newId, selectEl) {
                ...
                if(this.items.length > 0 && this.items[0].product_id) {
                    const result = await window.confirmDialog({
                        ...
                        text: 'Mengganti supplier akan mereset daftar barang yang sudah diinput. Lanjutkan?',
                        icon: 'warning',
            ...
            saveDraft(showNotif = false) {
                if(!this.supplierId && this.items.length === 0) return;
                ...
                localStorage.setItem('po_create_draft', JSON.stringify(draftData));
                
                if(showNotif && typeof showToast === 'function') {
                    showToast('Draft berhasil disimpan.', 'success');
                }
            },

            checkDraft() {
                const draft = localStorage.getItem('po_create_draft');
                if(draft) {
                    const data = JSON.parse(draft);
                    if(typeof showToast === 'function') {
                        showToast('Draft pekerjaan sebelumnya ditemukan.', 'info');
                    }
            ...
            restoreDraft(data) {
                ...
                if(this.summary.rounding_display && !this.summary.rounding) {
                    this.summary.rounding = parseFloat(this.summary.rounding_display.replace(/\./g, '').replace(',', '.')) || 0;
                }

                const taxSelect = document.getElementById('tax_select');
                if(taxSelect && taxSelect.tomselect) {
                    taxSelect.tomselect.setValue(this.summary.tax_id, true);
                }
            ...
            async clearDraft() {
                const result = await window.confirmDialog({
                    title: 'Hapus Draft & Reset?',
                    text: 'Apakah Anda yakin? Draft yang tersimpan di browser akan dihapus permanen dan formulir akan dikosongkan.',
                    icon: 'warning',
                ...
                if (result.isConfirmed) {
                    localStorage.removeItem('po_create_draft');
            ...
            addDiscountLevel(index) {
                this.items[index].discounts.push(0); 
            },
            ...
            parseBulkString(str) {
                if (!str) return [];
                const separators = /[+,]/;
                return str.toString().split(separators)
            ...
            applyBulkDiscount(target) {
                ...
                if(typeof showToast === 'function') showToast('Diskon massal diterapkan.', 'success');
            },

            toggleDppFactor() {
                if (this.summary.use_custom_dpp) {
                    if (!this.summary.dpp_factor_input) {
                        this.summary.dpp_factor_input = '11/12';
                    }
                    this.evaluateFraction();
                } else {
            ...
            calculateSummary() {
                ...
                // Menggunakan window.taxes untuk mencari rate pajak
            ...
            submitForm(e) {
                if (!this.supplierId) { alert('Silakan pilih Supplier.'); return; }
                const validItems = this.items.filter(i => i.product_id !== '');
                if (validItems.length === 0) {
                    if(typeof showToast === 'function') showToast('Pilih minimal 1 produk.', 'error');
