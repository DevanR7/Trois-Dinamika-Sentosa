...
<div class="max-w-2xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <nav class="flex text-sm text-slate-500 mb-1">
                <a href="{{ route('admin.bank-reconciliations.index') }}" class="hover:text-indigo-600 transition-colors">Rekonsiliasi</a>
    ...
    <div class="dashboard-card p-0 overflow-hidden shadow-lg border-0 ring-1 ring-slate-900/5">
        ...
        <div class="p-6 md:p-8">
            <form action="{{ route('admin.bank-reconciliations.store') }}" method="POST" id="recon-create-form">
                @csrf
                ...
                <div class="mt-8 pt-6 border-t border-slate-100 flex justify-end gap-3">
                    <a href="{{ route('admin.bank-reconciliations.index') }}" class="h-[48px] px-6 bg-white border border-slate-300 rounded-lg text-slate-600 text-sm font-bold hover:bg-slate-50 transition-all flex items-center justify-center shadow-sm">
                        Batal
                    </a>
