        ...
        <div class="col-lg-10">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-success text-white">
                    <h4 class="mb-0">Edit Pesanan: {{ $salesOrder->order_number }}</h4>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('sales-orders.update', $salesOrder->order_id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="client_id" id="client_id" class="form-select" required>
                                    @foreach ($clients as $client)
                                        <option value="{{ $client->client_id }}" {{ old('client_id', $salesOrder->client_id) == $client->client_id ? 'selected' : '' }}>
                                            {{ $client->client_name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row justify-content-between">
                            <div class="col-md-6">
                                ...
                                <textarea class="form-control" name="notes" id="notes" rows="3">{{ old('notes', $salesOrder->notes) }}</textarea>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const salesOrderItems = @json($salesOrder->items);
        ...
        function addProductRow(item = null) {
            ...
            if (item) {
                ...
                // INI BAGIAN KUNCI UNTUK MEMPERBAIKI 'undefined'
                // Kita ambil harga historis dari data item
                const historicPrice = item.price_per_unit;
                priceDisplay.value = formatRupiah(historicPrice);
                priceRaw.value = historicPrice;
            } else {
