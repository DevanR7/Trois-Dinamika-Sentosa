...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div class="flex gap-3 w-full sm:w-auto">
            ...
            {{-- Opsi Lanjutan (Dropdown) --}}
            @if (!in_array($order->status, ['invoiced', 'rejected']))
            <div class="relative group">
                <button class="h-[48px] px-6 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
                    <i class="material-icons text-[18px]">settings</i> Opsi <i class="material-icons text-[18px]">expand_more</i>
                </button>
                <div class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-slate-100 hidden group-hover:block z-50 overflow-hidden">
                    @can('create', App\Models\SalesInvoice::class)
                        <a href="{{ route('invoices.createFromOrder', $order) }}" class="flex items-center px-4 py-3 text-sm text-emerald-600 hover:bg-emerald-50 transition font-medium">
                            <i class="material-icons text-lg mr-2">receipt_long</i> Buat Invoice
                        </a>
                    @endcan
                    
                    @can("update", $order)
                        <a href="{{ route('sales-orders.edit', $order->order_id) }}" class="flex items-center px-4 py-3 text-sm text-amber-600 hover:bg-amber-50 transition border-t border-slate-50">
                            <i class="material-icons text-lg mr-2">edit</i> Edit Pesanan
                        </a>
                    @endcan
                    
                    @can("delete", $order)
                        <form action="{{ route('sales-orders.destroy', $order->order_id) }}" method="POST" class="delete-form w-full">
                            @csrf @method('DELETE')
                            <button type="submit" class="w-full text-left flex items-center px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition border-t border-slate-50">
            ...
            @if ($order->status == 'invoiced' && $order->invoice_id)
                <a href="{{ route('invoices.show', $order->invoice_id) }}" class="h-[48px] px-6 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 transition-all shadow-md flex items-center justify-center gap-2">
                    ...
                    <div class="space-y-4 border-l border-slate-100 pl-0 md:pl-8">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-semibold text-slate-800 flex items-center gap-2">
                                <i class="material-icons text-slate-400 text-[16px]">event</i> {{ $order->order_date->format('d F Y') }}
                            </span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-xs font-bold text-slate-500 uppercase">Sales Person</span>
                            <span class="text-sm font-semibold text-slate-800 flex items-center gap-2">
                                <i class="material-icons text-slate-400 text-[16px]">person</i> {{ $order->sales->full_name ?? 'N/A' }}
                            </span>
                        </div>
                        <div class="flex justify-between items-center">
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="dashboard-card p-6 shadow-lg sticky top-6 border-t-4 border-indigo-500">
                ...
                <div class="flex flex-col items-center justify-center mb-8 bg-slate-50 p-6 rounded-xl border border-slate-100">
                    ...
                    <span class="text-3xl font-bold text-indigo-700 font-mono tracking-tight">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                </div>
