...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <a href="{{ route('sales-orders.index') }}" class="h-[48px] px-6 bg-white border border-slate-300 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="dashboard-card p-0 overflow-hidden shadow-sm min-h-[300px]">
                    ...
                    <div class="p-6 border-t border-slate-100 bg-slate-50/30">
                        ...
                        <textarea class="form-textarea bg-white" name="notes" id="notes" rows="2" placeholder="Contoh: Kirim sebelum jam 5 sore...">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addProductRow(item = null) {
        ...
        // Unique Names for Laravel Array Validation
        productSelect.name = `items[${productIndex}][product_id]`;
        quantityInput.name = `items[${productIndex}][quantity]`;
        priceRaw.name = `items[${productIndex}][price]`; // Optional if price comes from DB
        ...
        // If item data exists (for Edit mode later), populate it
        if(item) {
             // Logic for edit mode population
        }
    }
