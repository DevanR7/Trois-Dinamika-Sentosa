    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-1">
                <a href="{{ route('announcements.index') }}" class="hover:text-indigo-600 transition-colors font-medium">Pengumuman</a>
                <i class="material-icons text-[14px] text-slate-400">chevron_right</i>
                <span class="text-slate-800 font-semibold">Baru</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Buat Pengumuman Baru</h1>
            <p class="text-slate-500 text-sm mt-1">Buat informasi baru untuk disiarkan ke klien.</p>
        </div>
    ...
    <form action="{{ route('announcements.store') }}" method="POST">
        ...
        <div class="dashboard-card p-0 overflow-hidden shadow-sm">
            <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/80 flex items-center gap-3">
                ...
                <h3 class="card-title mb-0">Form Pengumuman</h3>
            </div>
            ...
            <div class="px-6 py-4 bg-slate-50/80 border-t border-slate-200 flex justify-end gap-3">
                ...
                <button type="submit" 
                    ...
                    <i class="material-icons text-[18px]">send</i> Simpan & Terbitkan
                </button>
            </div>
        </div>
    </form>
