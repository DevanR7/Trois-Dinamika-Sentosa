    ...
    <form action="{{ route('bank-reconciliations.update', $bankReconciliation) }}" method="POST" id="recon-form">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <div class="flex gap-3 w-full sm:w-auto">
                <a href="{{ route('bank-reconciliations.index') }}" class="h-[48px] px-6 bg-white border border-slate-300 rounded-lg font-bold text-sm text-slate-600 hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
        ...
        <div class="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-5 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
            ...
            @if(!$isLocked)
            <a href="{{ route('manual-journals.create') }}" target="_blank" class="inline-flex items-center px-5 py-2.5 bg-white border border-amber-300 text-amber-700 rounded-lg text-sm font-bold hover:bg-amber-100 transition shadow-sm whitespace-nowrap">
