    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <nav class="flex items-center gap-2 text-sm text-slate-500 mb-1">
                <a href="{{ route('client-order-reviews.index') }}" class="hover:text-indigo-600 transition-colors font-medium">Review Pesanan</a>
        ...
        <div class="flex gap-3 w-full sm:w-auto">
            <a href="{{ route('client-order-reviews.index') }}" class="h-[48px] px-6 bg-white border border-slate-300 rounded-lg text-slate-700 text-sm font-bold hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
            ...
            <div class="dashboard-card p-6 shadow-xl sticky top-6 border-t-4 {{ $order->status == 'pending_review' ? 'border-amber-400' : ($order->status == 'rejected' ? 'border-red-500' : 'border-emerald-500') }}">
                ...
                @if($order->status == 'pending_review')
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('invoices.createFromOrder', $order->order_id) }}" 
                ...
                @elseif($order->status == 'invoiced' || $order->status == 'approved')
                    <div class="text-center py-6">
                        ...
                        @if($order->invoice_id)
                        <a href="{{ route('invoices.show', $order->invoice_id) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 transition shadow-sm">
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                        ...
                        <div class="mt-2">
                            ...
                            <form action="{{ route('client-order-reviews.reject', $order->order_id) }}" method="POST" class="mt-4">
                                @csrf
