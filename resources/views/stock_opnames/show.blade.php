    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <nav class="flex text-sm text-slate-500 mb-1">
                <a href="{{ route('stock-opnames.index') }}" class="hover:text-indigo-600 transition-colors">Opname</a>
        ...
        <div class="flex gap-3 w-full sm:w-auto">
            ...
            <form action="{{ route('stock-opnames.destroy', $stockOpname->opname_id) }}" method="POST" class="form-confirm inline-block">
                @csrf @method('DELETE')
                <button type="submit" 
            ...
            <a href="{{ route('stock-opnames.index') }}" class="h-[48px] px-6 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
