 ...
 <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    ...
    <div class="btn-group">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
            ...
            {{-- Pastikan ada spasi setelah @if di sini juga --}}
            @if (in_array($purchaseOrder->status, ['draft', 'ordered']))
            <li><hr class="dropdown-divider"></li>
            <li>
    ...
    <div class="col-md-6 text-md-end">
        ...
        {{-- TAMPILKAN NO FAKTUR SUPPLIER JIKA ADA --}}
        <p class="mb-1">
    ...
    <a href="#" data-bs-toggle="modal" data-bs-target="#supplierInvoiceModal" class="text-decoration-none" title="Klik untuk input/edit">
        ...
        @endif
                    ...
                    {{-- PERBAIKAN 3: Memperbarui logika status pembayaran --}}
            ...
            <div class="row mt-4">
                ...
                <div class="col-md-5">
                    <h5 class="fw-semibold mb-3">Ringkasan Pembelian</h5>
                    <div class="border rounded p-3">
                        <div class="d-flex justify-content-between mb-2">
                            <span>Subtotal Barang</span>
                            <span class="fw-medium">Rp {{ number_format($purchaseOrder->subtotal ?? 0, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            <span>
                                ...
                                @if($purchaseOrder->disc_fee_percent > 0)
                                    <small class="text-muted">({{ $purchaseOrder->disc_fee_percent }}%)</small>
                                @endif
                            </span>
                            <span class="text-danger">(-) Rp {{ number_format($purchaseOrder->disc_fee_amount ?? 0, 0, ',', '.') }}</span>
                        </div>

                        <div class="d-flex justify-content-between mb-2">
                            <span>Diskon Pembulatan</span>
                            <span class="text-danger">(-) Rp {{ number_format($purchaseOrder->rounding_discount_amount ?? 0, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            <span>
                                ...
                                @if($purchaseOrder->custom_dpp_factor)
                                    <small class="d-block text-muted">Faktor: {{ $purchaseOrder->custom_dpp_factor }}</small>
                                @elseif($purchaseOrder->tax)
                                     <small class="d-block text-muted">Faktor: Default PPN Inklusif</small>
                                @endif
                            </span>
                            <span class="fw-medium">Rp {{ number_format($purchaseOrder->dpp ?? 0, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="d-flex justify-content-between fw-bold fs-5">
                            ...
                            <span>Rp {{ number_format($purchaseOrder->total_amount ?? 0, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="paymentModal" tabindex="-1" aria-labelledby="paymentModalLabel" aria-hidden="true">
    ...
    <div class="d-flex justify-content-between">
        ...
        <span>Rp {{ number_format($purchaseOrder->amount_paid ?? 0, 0, ',', '.') }}</span>
    </div>
    <hr class="my-1">
    <div class="d-flex justify-content-between fw-bold">
        <span>Sisa Tagihan:</span>
        <span>Rp {{ number_format(($purchaseOrder->total_amount ?? 0) - ($purchaseOrder->amount_paid ?? 0), 0, ',', '.') }}</span>
    </div>
</div>
                    <div class="mb-3">
    ...
    {{-- Input yang dilihat pengguna, untuk tampilan --}}
    <input type="text" class="form-control" id="amount-formatted" required>

    {{-- Input tersembunyi, untuk mengirim data angka ke server --}}
    ...
    if (amountFormattedInput) {
        ...
        if (addPaymentBtn) {
            ...
            amountFormattedInput.addEventListener('autoNumeric:rawValueModified', function(event) {
                ...
                const remainingBalance = {{ ($purchaseOrder->total_amount ?? 0) - ($purchaseOrder->amount_paid ?? 0) }};
