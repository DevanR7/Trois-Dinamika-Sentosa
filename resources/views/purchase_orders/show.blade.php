                ...
                <div class="row mt-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="border rounded p-3">
                            ...
                            <hr class="my-1">
                            @php
                                // ✅ PERBAIKAN: Ambil total retur yang dipotong (sudah di-cache di kolom total_returned)
                                $totalReturDipotongPO = $purchaseOrder->total_returned;

                                // Sisa hutang HANYA dikurangi retur yang dipotong
                                $sisaUtang = $purchaseOrder->total_amount - $totalReturDipotongPO - $purchaseOrder->amount_paid;
                                
                                // (Opsional) Hitung retur yang jadi deposit, hanya untuk info
                                $totalReturDeposit = $purchaseOrder->returns
                                    ->where('return_handling_type', 'store_as_deposit')
                                    ->sum('total_amount');
                            @endphp

                            <div class="d-flex justify-content-between text-warning mb-2"><span>Total Retur (Potong Tagihan)</span><span>(-) Rp {{ number_format($totalReturDipotongPO, 0, ',', '.') }}</span></div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('purchase-orders.payments.store', $purchaseOrder->po_id) }}" method="POST">
                    ...
                    // ✅ PERBAIKAN DI SINI
                    $saldoDepositSupplier = $purchaseOrder->supplier->balance ?? 0; // <-- DIUBAH
                @endphp
    ...
    <script>
        ...
        // ✅ PERBAIKAN DI SINI
        const currentDebitBalancePO = {{ $purchaseOrder->supplier->balance ?? 0 }}; // <-- DIUBAH
