        ...
        <div class="col-lg-10">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-success text-white">
                    <h4 class="mb-0">Edit Pesanan Pembelian: {{ $purchaseOrder->po_number ?? 'PO-'.$purchaseOrder->po_id }}</h4>
                </div>
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('purchase-orders.update', $purchaseOrder->po_id) }}" method="POST" id="po-form">
                        ...
                        {{-- Bagian Header Form (Supplier, Tanggal, dll) --}}
                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <div class="mb-3 mt-4">
                                    ...
                                    <textarea class="form-control" name="notes" id="notes" rows="3">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end mt-4">
                            ...
                            <button type="submit" class="btn btn-success" id="submit-btn">Update Pesanan</button>
                        </div>
                    </form>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Fungsi-fungsi Helper dan Kalkulasi
    ...
    function addProductRow(shouldCalculate = true) {
        ...
        $(productSelect).select2({ placeholder: '-- Pilih Produk --', theme: 'bootstrap-5', dropdownParent: $(productSelect).parent() }).on('select2:select', function(e) {
        ...
        newRow.querySelector('.remove-product-btn').onclick = () => { $(productSelect).select2('destroy'); newRow.remove(); if (shouldCalculate) calculateTotals(); };
        const updatePrice = () => {
    ...
    // Fungsi untuk mengisi data PO yang sudah ada
    function populateExistingItems() {
        if (existingPoItems && existingPoItems.length > 0) {
            existingPoItems.forEach(item => {
                const newRow = addProductRow(false); // Buat baris baru tanpa trigger kalkulasi
                const productSelect = newRow.querySelector('.product-select');
                const quantityInput = newRow.querySelector('.quantity');
                const formattedPriceInput = newRow.querySelector('.purchase-price-formatted');
                ...
                formattedPriceInput.value = formatThousands(item.price_per_unit);
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (!document.getElementById('supplier_id').value) { e.preventDefault(); alert('Harap pilih supplier terlebih dahulu.'); return; }
