...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-end gap-4 mb-8">
        ...
        <a href="{{ route('purchase-orders.create') }}" class="h-[48px] px-8 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md transition-all flex items-center justify-center gap-2 group hover:-translate-y-0.5">
        ...
        <form action="{{ route('purchase-orders.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                ...
                <div class="md:col-span-2 flex gap-2">
                    ...
                    <a href="{{ route('purchase-orders.index') }}" class="h-[48px] w-[48px] flex items-center justify-center bg-white border border-slate-300 text-slate-500 hover:text-indigo-600 font-medium rounded-lg shadow-sm transition" title="Reset">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 text-sm">
                                ...
                                <div class="lg:col-span-4 flex flex-col gap-2 justify-center">
                                    ...
                                    <div class="flex gap-2">
                                        <a href="{{ route('purchase-orders.show', $po->po_id) }}" class="flex-1 px-3 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-50 transition text-center flex items-center justify-center gap-1">
                                            <i class="material-icons text-[16px]">visibility</i> Detail
                                        </a>
                                        <a href="{{ route('purchase-orders.pdf', $po->po_id) }}" target="_blank" class="flex-1 px-3 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-50 transition text-center flex items-center justify-center gap-1">
                                    ...
                                    @if(in_array($po->status, ['draft', 'ordered']))
                                        <div class="flex gap-2 mt-1">
                                            <a href="{{ route('purchase-orders.edit', $po->po_id) }}" class="flex-1 px-3 py-2 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg text-xs font-bold hover:bg-amber-100 transition text-center flex items-center justify-center gap-1">
                                                <i class="material-icons text-[16px]">edit</i> Edit
                                            </a>
                                            
                                            <form action="{{ route('purchase-orders.cancel', $po->po_id) }}" method="POST" class="delete-form flex-1">
                                                @csrf
                                                <button type="submit" 
