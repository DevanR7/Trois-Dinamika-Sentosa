...
<div class="max-w-7xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-end gap-4 mb-8">
        ...
        <a href="{{ route('payment-methods.index') }}" class="h-[48px] px-6 bg-white border border-slate-300 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="pr-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    ...
                                    <form action="{{ route('payment-methods.archived.restore', $method->payment_method_id) }}" method="POST" class="form-confirm inline-block">
                                        @csrf
                                        <button type="submit" 
                                    ...
                                    <form action="{{ route('payment-methods.archived.forceDelete', $method->payment_method_id) }}" method="POST" class="delete-form inline-block">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
