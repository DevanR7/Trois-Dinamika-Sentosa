...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            ...
            <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Tambah Tarif Pajak</h2>
            <p class="text-sm text-gray-500 mt-1">Definisikan tarif PPN, PPh, atau pajak lainnya.</p>
        </div>
    ...
    <form action="{{ route('taxes.store') }}" method="POST">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center gap-2">
                <i class="material-icons text-indigo-500">percent</i>
                <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wider">Form Data Tarif</h3>
            </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-input block w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: PPN, PPh 23" required>
                </div>
                ...
                <div>
                    ...
                    <div class="relative rounded-md shadow-sm w-1/2">
                        <input type="number" step="0.01" name="rate" id="rate" value="{{ old('rate') }}" class="form-input block w-full rounded-lg border-gray-300 pr-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm font-bold text-right" placeholder="11.00" required>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                            <span class="text-gray-500 sm:text-sm">%</span>
                        </div>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">Gunakan titik desimal jika diperlukan (misal: 11.50).</p>
                </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-lg font-bold text-sm text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-md transition">
                    <i class="material-icons text-lg mr-2">save</i> Simpan Tarif
                </button>
            </div>
        </div>
    </form>
