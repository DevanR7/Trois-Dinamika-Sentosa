                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse ($invoices as $invoice)
                                <tr>
                                     <th>{{ $loop->iteration + $invoices->firstItem() - 1 }}</th> 
                                    <td>
                                        <a
...
<td class="text-end">Rp {{ number_format($invoice->total_amount, 0, ',', '.') }}</td>
                            <td class="text-end fw-bold {{ ($invoice->total_amount - $invoice->amount_paid > 0) ? 'text-danger' : 'text-success' }}">
                                ...
                                @else
                                    <span class="badge bg-secondary">{{ Str::title($invoice->status) }}</span>
                                @endif
                            </td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
        <a href="{{ route('invoices.show', $invoice->invoice_id) }}" class="btn btn-sm btn-outline-info" title="Detail"><i class="bi bi-eye"></i> Detail</a>

        {{-- Tombol Edit & Batalkan hanya muncul jika statusnya 'unpaid' --}}
        @if ($invoice->status == 'unpaid')
            ...
            <form class="delete-form" action="{{ route('invoices.cancel', $invoice->invoice_id) }}" method="POST">
                @csrf
