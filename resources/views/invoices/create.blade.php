        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="dashboard-card p-6 shadow-sm">
                    ...
                    <textarea name="notes" id="notes" rows="3" class="form-textarea bg-yellow-50/30" placeholder="Catatan untuk klien...">{{ old('notes') }}</textarea>
                </div>
                ...
                <div class="dashboard-card p-6 shadow-lg sticky top-6 border-t-4 border-indigo-500">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between text-slate-600">
                            ...
                            <span class="font-bold text-slate-900" id="subtotal-display">Rp 0</span>
                        </div>

                        {{-- Diskon --}}
                        <div class="flex items-center justify-between gap-2">
                            <label for="discount_percentage" class="text-xs text-slate-500">Diskon (%)</label>
                            <input type="number" name="discount_percentage" id="discount_percentage" value="{{ old('discount_percentage', 0) }}" class="w-16 text-right text-xs border-slate-300 rounded form-input py-1 h-8" min="0" max="100">
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <div id="tax-options" class="space-y-2 bg-slate-50 p-3 rounded-lg border border-slate-100">
                                ...
                                @empty
                                    <p class="text-xs text-slate-400 italic">Tidak ada pajak aktif.</p>
                                @endforelse
                            </div>
                            {{-- Breakdown Pajak --}}
                    ...
                    <div class="flex flex-col gap-3 mt-8">
                        <button type="submit" class="w-full h-[48px] bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md transition-all flex justify-center items-center gap-2 group hover:-translate-y-0.5">
                            <i class="material-icons text-[20px] group-hover:scale-110 transition-transform">save</i> Simpan Invoice
                        </button>
                        <a href="{{ route('invoices.index') }}" class="w-full h-[48px] bg-white border border-slate-300 text-slate-700 text-sm font-bold rounded-lg hover:bg-slate-50 transition-all flex justify-center items-center shadow-sm">
                            Batal
                        </a>
                    </div>
                </div>
...
<template id="product-row-template">
    <tr class="hover:bg-slate-50 transition-colors group">
        ...
        <td class="py-3 align-top text-right font-bold text-slate-900 text-sm align-middle font-mono">
...
{{-- TEMPLATE ROW BIAYA TAMBAHAN (HIDDEN) --}}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addProductRow(item = null) {
        ...
        // Pre-fill (jika ada data dari Order)
        if (item) {
