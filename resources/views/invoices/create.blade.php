...
<div class="container py-4">
    ...
    <select name="client_id" id="client_id" class="form-select" required>
        ...
        @foreach ($clients as $client)
            <option value="{{ $client->client_id }}" @selected(old("client_id", isset($salesOrder) ? $salesOrder->client_id : '') == $client->client_id)>
                {{ $client->client_name }}
            </option>
        @endforeach
    </select>

    {{-- TAMBAHAN: Hidden input jika membuat dari Sales Order --}}
    @if (isset($salesOrder))
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const salesOrderClientId = @json(isset($salesOrder) ? $salesOrder->client_id : null);

    if (salesOrderClientId) {
        // Set nilai dropdown klien
        $('#client_id').val(salesOrderClientId).trigger('change');
        // Kunci (disable) dropdown agar tidak bisa diubah
    ...
    function addProductRow() {
        ...
        productIndex++;
        return newRow; // <-- [PERBAIKAN] Tambahkan return agar bisa diakses
    }
    ...
    // [LANGKAH 2] Logika inisialisasi halaman yang baru
