...
<div class="max-w-7xl mx-auto space-y-6 pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-end gap-4">
        ...
        <a href="{{ route('products.create') }}" class="h-[48px] px-6 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-bold shadow-md transition-all flex items-center gap-2">
        ...
        <form action="{{ route('products.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                ...
                <div class="md:col-span-3 flex gap-2">
                    ...
                    <a href="{{ route('products.index') }}" class="h-[48px] w-[48px] flex items-center justify-center bg-white border border-slate-300 hover:border-indigo-500 hover:text-indigo-600 rounded-lg transition-all" title="Reset">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="pl-6 py-3 w-[35%]">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <a href="{{ route('products.show', $product->product_id) }}" class="text-sm font-bold text-slate-700 line-clamp-1 group-hover:text-indigo-600 transition-colors">
                            ...
                            <td class="text-center pr-6 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    {{-- Detail --}}
                                    <a href="{{ route('products.show', $product->product_id) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white text-slate-500 border border-slate-200 hover:text-indigo-600 hover:border-indigo-200 transition-colors shadow-sm" title="Detail">
                                        <i class="material-icons text-[16px]">visibility</i>
                                    </a>
                                    
                                    {{-- Edit --}}
                                    @can('update', $product)
                                    <a href="{{ route('products.edit', $product->product_id) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white text-slate-500 border border-slate-200 hover:text-amber-600 hover:border-amber-200 transition-colors shadow-sm" title="Edit">
                                    ...
                                    @can('delete', $product)
                                    <form action="{{ route('products.destroy', $product->product_id) }}" method="POST" class="delete-form">
                                        @csrf @method('DELETE')
