...
<div class="max-w-6xl mx-auto pb-20 animate-enter">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div class="flex gap-2 w-full sm:w-auto">
            <a href="{{ route('products.index') }}" class="px-4 py-2 bg-white border border-slate-300 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center gap-2">
                <i class="material-icons text-[18px]">arrow_back</i> Kembali
            </a>
            
            @can('update', $product)
            <a href="{{ route('products.edit', $product->product_id) }}" class="px-4 py-2 bg-white border border-slate-300 text-indigo-600 rounded-lg text-sm font-bold hover:bg-indigo-50 hover:border-indigo-200 transition-all shadow-sm flex items-center justify-center gap-2">
                <i class="material-icons text-[18px]">edit</i> Edit
            </a>
            @endcan

            @can('delete', $product)
            <form action="{{ route('products.destroy', $product->product_id) }}" method="POST" class="delete-form inline-block">
                @csrf @method('DELETE')
        ...
        <div class="space-y-8">
            ...
            <div class="dashboard-card p-6">
                ...
                <div class="flex justify-between items-end mb-4">
                    <div>
                        <span class="text-4xl font-bold text-slate-800">{{ $product->stock_quantity }}</span>
