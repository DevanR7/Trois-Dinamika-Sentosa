...
<div class="max-w-4xl mx-auto animate-enter pb-20">
    ...
    <div class="text-center mb-10 pt-6">
        ...
        <p class="text-slate-500 mt-2 text-sm max-w-lg mx-auto">Koreksi tagihan penjualan (Nota Debet/Kredit) untuk Invoice yang sudah berjalan dengan aman dan tercatat.</p>
    </div>

    {{-- CARD WIZARD --}}
    <div class="dashboard-card p-0 overflow-hidden relative">
        <div class="p-8">
            ...
            <div class="relative z-10">
                ...
                <div class="sm:ml-12">
                    ...
                    <div class="relative">
                        <select id="sales_invoice_id" class="w-full select2-basic" data-placeholder="Ketik No. Invoice atau Nama Klien...">
                            <option value="" disabled {{ !$preselectedInvoiceId ? 'selected' : '' }}></option>
                    ...
                    <div class="mt-3 flex items-start gap-2 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
                        <i class="material-icons text-blue-500 text-sm mt-0.5">info</i> 
                        <p class="text-xs text-blue-700 leading-relaxed">
                            Hanya Invoice dengan status <b>Unpaid</b> atau <b>Partially Paid</b> yang dapat disesuaikan. Invoice lunas tidak akan muncul disini.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div id="method-selection" class="mt-10 sm:ml-12 border-t border-dashed border-slate-200 pt-8 hidden opacity-0 transition-all duration-500 relative">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <a href="#" id="link-auto" class="group relative block h-full">
                        <div class="h-full p-6 rounded-xl border border-slate-200 bg-white hover:border-indigo-500 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                            <div class="absolute top-4 right-4">
                                <span class="px-2 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-bold rounded uppercase border border-emerald-100 tracking-wide">Recommended</span>
                            </div>
                    ...
                    <a href="#" id="link-manual" class="group relative block h-full">
                        <div class="h-full p-6 rounded-xl border border-slate-200 bg-white hover:border-slate-400 hover:bg-slate-50 hover:shadow-md transition-all duration-300">
                            ...
                            <p class="text-sm text-slate-500 leading-relaxed">
                                Input nominal Nota Kredit atau Nota Debet secara langsung tanpa merinci item per item.
                            </p>
                        </div>
                    </a>
        ...
        <div class="bg-slate-50 px-8 py-4 border-t border-slate-200 flex justify-center">
            <a href="{{ route('invoices.index') }}" class="text-xs font-bold text-slate-400 hover:text-slate-600 uppercase tracking-widest transition-colors">
                Batal & Kembali
            </a>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    // Init Select2 Global sudah ada di app.js via class .select2-basic, 
    // tapi kita perlu event listener spesifik di sini.
    ...
    // Handle preselection if any
    const preselectedId = "{{ $preselectedInvoiceId ?? '' }}";
    if (preselectedId) {
        // Trigger change manual karena select2 init mungkin async atau sudah jalan
