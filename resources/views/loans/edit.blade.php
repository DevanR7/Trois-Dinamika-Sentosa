        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header">
                    <h2 class="fw-bold mb-0">Edit Pinjaman</h2>
                </div>
                <div class="card-body">
                    @if ($loan->payments()->exists())
                        <div class="alert alert-danger">
                            <i class="bi bi-exclamation-triangle-fill"></i> Pinjaman ini tidak bisa diedit karena sudah memiliki riwayat pembayaran.
                        </div>
                    @endif
                    
                    <form action="{{ route('loans.update', $loan) }}" method="POST" {{ $loan->payments()->exists() ? 'disabled' : '' }}>
                        @csrf
                        @method('PUT')
