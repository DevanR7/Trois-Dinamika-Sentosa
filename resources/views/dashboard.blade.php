...
<div class="max-w-7xl mx-auto space-y-6 pb-10 animate-enter">
    ...
    <div class="flex flex-col md:flex-row justify-between items-end gap-4 mb-2">
        ...
        @if($canViewFinancials)
        <form action="{{ route('dashboard') }}" method="GET" class="w-full md:w-48">
            ...
            <div class="p-4 flex-1 overflow-y-auto custom-scrollbar">
                @if(array_sum($pendingActions) > 0)
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('invoices.index', ['status' => 'draft']) }}" class="action-item action-item-indigo group">
                        ...
                        <a href="{{ route('purchase-orders.index') }}" class="action-item action-item-blue group">
                            <div class="flex items-center gap-3">
                        ...
                        <a href="{{ route('payment-clearance.index') }}" class="action-item action-item-amber group">
                            <div class="flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-5 dashboard-card p-0 flex flex-col">
            <div class="p-5 border-b border-slate-100 flex justify-between items-center">
                ...
                <a href="{{ route('products.index') }}" class="text-[11px] font-bold text-indigo-600 hover:text-indigo-800 tracking-wide border-b border-dashed border-indigo-300 hover:border-indigo-600">LIHAT SEMUA</a>
            </div>
    ...
    <div class="dashboard-card p-0 overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/30">
            ...
            @if($canViewFinancials)
            <form action="{{ route('dashboard') }}" method="GET" class="w-full sm:w-64">
            ...
            <table class="dashboard-table w-full">
                ...
                <tbody>
                    @forelse ($latestOrders as $order)
                        <tr>
                            <td class="pl-6">
                                <a href="{{ route('sales-orders.show', $order->order_id) }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-800 font-mono tracking-tight">
                                    {{ $order->order_number }}
                                </a>
                            </td>
